...
def verify_quote(enclave_uuid, user_uuid, user_pub_key, enclave_pub_key, sharing_pub_key, sgx_quote):
    ...
    # Hash user data 
    #hash_input = '{0}{1}{2}{3}{4}'.format(enclave_uuid, user_pub_key, enclave_pub_key, user_uuid, sharing_pub_key).encode()
    #hash_value = hashlib.sha256(hash_input).digest()
    hash_value = hashlib.sha256()
    hash_value.update(enclave_uuid.encode('UTF-8'))
    hash_value.update(user_pub_key.encode('UTF-8'))
    hash_value.update(enclave_pub_key.encode('UTF-8'))
    hash_value.update(user_uuid.encode('UTF-8'))
    hash_value.update(sharing_pub_key.encode('UTF-8'))
    hash_value.digest()
